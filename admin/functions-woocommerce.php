...
/**
 * Create orders menu pages.
 */
if ( ! function_exists( 'wl_create_orders_menu_page_function' ) ) {
	function wl_create_orders_menu_page_function() {
		...
		add_menu_page(
			...
			$orders ? __( 'Orders', 'woocommerce' ) . sprintf( '&nbsp;<span class="awaiting-mod">%d</span>',
					count( $orders ) ) : __( 'Orders', 'woocommerce' ),
			'edit_shop_orders',
	...
	add_action( 'admin_menu', 'wl_create_orders_menu_page_function' );
}
...
/**
 * Disable reviews in WooCommerce
 */
if ( ! function_exists( 'wl_disable_woocommerce_reviews' ) ) {
	function wl_disable_woocommerce_reviews() {
		remove_post_type_support( 'product', 'comments' );
		remove_submenu_page( 'edit.php?post_type=product', 'product-reviews' );
	}

	//add_action( 'admin_menu', 'wl_disable_woocommerce_reviews' );
}
	...
	function date_setting_field() {
		...
		woocommerce_wp_text_input(
			array(
				...
				'label'         => __( 'Datę szkolenia', WL_THEME_ADMIN_DOMAIN ),
		...
		function enabling_date_picker() {
			...
			wp_enqueue_style( 'jquery-ui',
				"http://ajax.googleapis.com/ajax/libs/jqueryui/1.8/themes/smoothness/jquery-ui.css", '', '', false );
		}
